...
`name` 只用于 Actions 日志和 Job Summary，应使用不包含真实邮箱的别名。每次运行后，`username` 会连同成功或失败状态写入仓库根目录 `README.md`；如果仓库公开，用户名也会公开。不要将密码写入仓库文件或工作流 YAML。

`socks5` 是可选字段。未配置、配置为 `null` 或留空时，该账号使用 GitHub Actions 网络直接登录。配置代理时只接受无认证的 `socks5://host:port`；不要把用户名密码写进 SOCKS5 URL。

如果 SOCKS5 无法创建浏览器连接、无法打开登录页或登录页返回错误状态，脚本会记录脱敏的错误原因，关闭代理连接，并使用 GitHub Actions 网络直连重试一次。登录页成功打开后发生的账号密码错误、验证码或页面操作错误不会触发直连重试。
...
- 密码字段：`#password`
- 提交按钮：`继续`
- 脚本会等待账号输入框或“使用邮箱或用户名登录”入口真正出现，再决定是否点击入口；不会只根据页面刚完成 DOM 加载就立即点击。
- 提交后先等待页面离开 `/login`，再访问受保护页面 `/console/topup`。
- 只有受保护页面返回成功状态且最终地址仍位于 `/console` 下，才判定该账号登录成功；被送回 `/login` 时判定失败。
- 如果页面控件仍未出现，日志会记录失败阶段、网络方式、HTTP 状态、去掉查询参数的最终 URL 和脱敏页面标题，不记录页面正文。
...
- 每次任务结束后，工作流会覆盖根目录 `README.md`，以“用户名、结果、时间”三列表格展示每个账号的最近一次登录结果。
- 每行时间是该账号本次登录成功或失败判定完成时的北京时间，格式为 `YYYY-MM-DD HH:mm:ss`。
- README 不再显示总体北京时间、总体运行结果或成功失败总数；只要存在失败账号，GitHub Actions 任务本身仍会标记为失败。
- 逐账号记录不包含密码、SOCKS5 地址或失败详情；如果登录脚本未能生成账号明细，README 会显示“登录步骤未生成逐账号结果”。
- 工作流使用 `github-actions[bot]` 将记录提交到本次运行所在分支，并使用并发组避免本工作流同时推送。
- `README.md` 正文只保留最新一条记录，但 Git 提交历史仍会保存以前的文件版本。
...
- Actions 日志只输出账号别名，并屏蔽解析出的用户名、密码和已配置的 SOCKS5 地址；诊断信息只保留必要的错误和页面元数据。按当前配置，README 会明文记录用户名、登录结果和北京时间。

## 回滚

删除 `.github/workflows/login.yml`、`README.md`、`login.mjs`、`package.json`、`package-lock.json`、`test/` 和本说明文件即可移除该自动登录功能。
